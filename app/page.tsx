            ...
            {rooms.map((room, i) => (
              <div key={i} className="border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
                <div className="h-80 rounded-2xl overflow-hidden">
                  <img src={room.img} alt={room.name} className={`w-full h-full object-cover ${room.name === 'Camera Matrimoniale Allegra' ? 'scale-125' : room.name === 'Camera Singola' ? 'scale-[0.6]' : ''}`} style={room.name === 'Camera Matrimoniale Allegra' ? {transformOrigin: '65% center'} : room.name === 'Camera Singola' ? {transformOrigin: '20% center'} : {}} />
                </div>
                <div className="p-4">
